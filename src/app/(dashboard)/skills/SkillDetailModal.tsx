...
import { X, Power, FileText, ExternalLink } from "lucide-react";
...
function ModalHeader({ skill, isToggling, onToggle, onClose }: ModalHeaderProps) {
  return (
    <div
      style={{
        padding: "24px",
...
function SkillMetadata({ skill }: { skill: Skill }) {
  return (
    <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "center" }}>
      ...
      <FileCountBadge count={skill.fileCount} />
      {!skill.enabled && <DisabledBadge />}
      {skill.agents && skill.agents.map((agent) => <AgentBadge key={agent} agent={agent} />)}
      {skill.homepage && <HomepageLink href={skill.homepage} />}
    </div>
  );
}
...
function FileCountBadge({ count }: { count: number }) {
  return <div className="badge-info">{count} archivos</div>;
}
...
function ModalContent({ skill }: ModalContentProps) {
  return (
